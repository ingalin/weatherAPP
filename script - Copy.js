...
app.makeCityArray = function () {
    for (let i = 0; i < localData.length; i++) {
        ...
        if (app.addCountryToArray !== "" && app.addNameToArray !== "" && app.addStateToArray !== "") {
            ...
            // app.cityArray.forEach(function (index) {
            //     if (eachCityObject.indexOf(index) == -1) {
            //         app.cityArray.push(eachCityObject);
            //     }
            // });
            // console.log(eachCityObject)

            let counter = 0;
            if (app.cityArray.length > 0) {
                for (let n = 0; n < app.cityArray.length; n++) {
                    if (app.cityArray[n].label === eachCityObject.label) {
                        counter = 1;
                        break;
                    }
                }
